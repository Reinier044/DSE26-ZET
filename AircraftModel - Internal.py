...
#Variables
DesAccel        = 0.7           #[m/s^2]Desired acceleration
Rvw             = Rmlg          #[m] Radius Vehicle Wheel
MLGTugW         = 690           #[kg] estimated system weight at MLG
NGTugW          = 0             #[kg] estimated system weight at NG
Slope           = 0.0           #[rad] 0.0300196631 max (aka 3%)
CGfslg          = 1/3           #[%] assumed position of cg wrt fuselage (0 is bottom, 1 is top)
LiftNG          = 0.0          #[m] How high the nosegear is lifted
LiftMLG         = 0.0          #[m] How high the main landing gear is lifted
...
NumWheels       = 4             #Number of wheels on driving axles
GearRatio       = 43.75         #Gear ratio
max_d           = -1.5          #Maximum deceleration -> should be negative valu5.144e!
max_v           = 12.3466667    #Maximum achievable velocity -> 30 kts (15.433), 25 kts (12.86)
...
MaxPower        = 210.5         #[kW] Selected engine MaxPower
AvgPower        = 56.18         #[kW] Selected engine avgPower
EngineRPM       = 4500          #RPM for selected engine
NumCycles       = 10            #Number of cycles to be done
...
if Taxi_with_ac == "yes":
    MRW = MRW + MLGTugW + NGTugW  #weight of combination 
else:
...
print("Taxi time to Polderbaan: ",tarray[-1]/60 ,"minutes")
...
while i< len(SectionCntrlForce):
    if SectionCntrlForce[i]>ThrustSetMax-100:
        ...
        Energy = Energy + AvgPower*SectionTime[i]
    i = i + 1
